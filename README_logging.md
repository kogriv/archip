...
Когда мы вызываем методы логеров (.debug(), .info() ...), логеры создают внутри себя специальные объекты - экземпляры класса LogRecord, и которые содержат всю нужную информацию о произошедшем событии (это могут быть имена ф-й, в которых вызваются методы логера, имя модуля, номер строки, время события, значения переменных и т.д.)
Полный список - в документации LogRecord attributes. Собственно, объекты LogRecord - это и есть те данные, которые пишуться в лог.
Затем эти объекты LogRecord направляются логером в обработчик(и) Handler, которые фильтруют эти записи и с нужными что то делают (обрабатывают: выводят в консоль, в лог-файл, отправляют не почту и т.д).
Обработчики создаются отдельными инструкциями и привызываются (добавляются) к объектам-логерам.
Дефолтный обработчик выводит обращение в консоль (StreamHandler)
Объекты LogRecord перед выводом в назначенное место необходимо преобразовать в строку. Для этого используются объекты класса Formatter. Форматировщики создаются отдельными инструкциями. Форматировщики привязываются к обработчикам.
...
# настройка форматировщика - создается как отдельная от логера сущность
...
По умолчанию обекты рут-логера имеют уровень `WARNING`. Уровень можно вывести на печать:
```python
print(logger.level)
```
...
Такой обработчик создается, но не закрепляется за объектом рут-логер.
...
- дефольный формат сообщения: `DEBUG:root:Some_massage`
...
Мы видим вывод от библиотек, используемых подмодулей. Все дело в том, что модуль `logging` использует паттерн `Singletone` (одиночка). Т.е. это глобальный модуль для всего нашего проекта. Объекты-логеры - глобальны. `logging.basicConfig(level='DEBUG')` создает глобальные установки для всех используемых нами модулей. Поскольку .basicConfig и создает настройки для рут-логера, и т.к. все логеры имеют своим предком рут-логер, то базовая конфигурация затронет все подмодули проекта.

Посмотреть все логеры, используемые в проекте:
```python
...
По умолчанию используется уровень, который установлен на предке. Установка уровня более низкого, чем на предке, при `propagate` = `True`, не изменит уровень. Уровень будет унаследован тогда от предка.
...
Все существующие обработчики каждого предка (при соответствии уровня методу-стартеру) обработают данное сообщение.
