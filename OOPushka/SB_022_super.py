# 3
# Объявите в программе базовый класс
# с именем Book, объекты которого
# создаются командой:
#
...
# year - год издания (целое число). В каждом
# объекте класса Book должны формироваться
# соответствующие локальные атрибуты:
# title, author, pages, year.
#
# Объявите дочерний класс DigitBook от
# класса Book, объекты которого
# создаются командой:
#
...
# frm - формат книги (строка:
# 'pdf', 'doc', 'fb2', 'txt'). В каждом
# объекте класса DigitBook должны формироваться
# соответствующие локальные атрибуты:
# title, author, pages, year, size, frm.
#
# Инициализация локальных атрибутов
# title, author, pages, year должна
# выполняться в базовом классе Book,
# а параметры size, frm инициализируются
...
# Каждый предмет на складе должен описываться
# базовым классом Thing. Объекты этого
...
#
# Для описания каждого конкретного вида
# предметов, создаются дочерние классы
...
#
# На основе класса Auto создаются дочерние
# классы Mercedes и Toyota, объекты
...
#
# Во всех объектах классов должны создаваться
# соответствующие локальные атрибуты:
# name, weight и т.д.
#
# Инициализация атрибутов должна выполняться
# в соответствующих классах (не должно
...
# 5
# Вам поручено организовать представление
# объектов для продажи в риэлтерских агентствах.
# Для этого в программе нужно объявить базовый
# класс SellItem, объекты которого
# создаются командой:
#
# item = SellItem(name, price)
#
# где
# name - название объекта продажи (строка);
# price - цена продажи (число: целое или вещественное).
#
# Каждые конкретные типы объектов описываются
# следующими классами, унаследованные от базового SellItem:
#
# House - дома;
# Flat - квартиры;
...
# формироваться соответствующие локальные
# атрибуты: name, price и т.д.
#
# Формирование атрибутов name и price должно
# выполняться в инициализаторе базового класса.
#
# Далее, объявить еще один класс с именем Agency,
...
#
# add_object(obj) - добавление нового
# объекта недвижимости для продажи (один
# из объектов классов: House, Flat, Land);
# remove_object(obj) - удаление объекта
# obj из списка объектов для продажи;
# get_objects() - возвращает список из
# всех объектов для продажи.
#
...
#     print(ret)
#
# Здесь Callback - это класс-декоратор
# с параметрами:
# path = '/' - маршрут;
# router_cls = Router - класс роутера.
#
# Декоратор Callback должен обеспечивать
# добавление функции (в примере index) в
# словарь app класса Router. Ключом словаря
# выступает маршрут (path), а значением -
# ссылка на декорируемую функцию. Для этого
# следует использовать метод add_callback
# класса Router.
#
# Затем, из роутера (Router) методом get
# выбирается ранее добавленная функция
# (в примере index), и если она существует,
...
# 7
# В программе объявлена функция integer_params
# для класса Vector, которая применяет к каждому
# методу класса декоратор integer_params_decorated:
#
...
#
# Декоратор integer_params_decorated должен проверять,
# чтобы все передаваемые аргументы в методы класса
# (кроме первого self) были целыми числами
# (имели тип int). Если это не так, то должно
