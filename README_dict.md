...
## dir()
`dir()` возвращает список строк, который представляет атрибуты объекта. Однако, это не словарь атрибутов объекта, а список строк имен атрибутов.
...
При использовании `dir()` вы не получаете словарь атрибутов объекта. Однако если вы хотите узнать, какие атрибуты у объекта, вы можете использовать `vars(obj)`, который возвращает словарь атрибутов объекта. Ключи этого словаря - это имена атрибутов объекта, а значения - сами атрибуты.
...
Ваш класс `Dummy` реализован так, что он имеет методы `__str__()`, `__repr__()`, `__hash__()` и `__eq__()`. Давайте рассмотрим ваш вывод:

Вывод `print("obj1:", obj1)` и `print("obj2:", obj2)`:  
Поскольку у вас определен метод `__str__()`, вывод `print()` использует именно его для строкового представления объектов `obj1` и `obj2`. Поэтому вы видите их имена, которые вы передали при создании объектов.

Вывод `print("Хэш obj1:", hash(obj1))` и `print("Хэш obj2:", hash(obj2))`:  
Метод `__hash__()` используется для вычисления хэш-значения объектов. Здесь вы видите их хэши, которые определяются по их именам.
Вывод `print(dict1)`:

Ваш словарь `dict1` содержит две пары: `{obj1: 1, obj2: 2}`. Однако, при выводе Python использует методы `__repr__()` для ключей. В вашем случае, поскольку ключи это объекты класса `Dummy` и для них определен метод `__repr__()`, Python выводит строковое представление, возвращаемое этим методом - `'dummy_class'`. Вот почему вы видите `{dummy_class: 1, dummy_class: 2}` в выводе словаря.
...
Этот метод сравнивает объекты на основе их длины имени и возвращает `True`, если длина имени обоих объектов находится в одном десятке (1-10, 11-20, более 20), иначе возвращает `False`.

## Провека на вхождение в список на основе `is` вместо `__eq__`
Вы можете создать функцию или декоратор для реализации функционала. Вот пример функции, которая проверяет наличие объекта в списке с использованием `is` вместо `==`:
```python
...
В Python нельзя напрямую переопределить синтаксис операторов, таких как `INS`, для использования в вашем контексте `obj2 INS objects_list`

## Сравнение словарей
Сравнение элементов словарей производится, как уже выяснили, с использованием интерфейса `__eq__` и `__hash__`.  
Для сложноорганизованных пользовательских классов прямое сравнение может дать неоднозначный результат.

Ниже приведен класс для более подробного разбора состава словарей с выводом информации которую дают функции `id(), hash(), type(), repr()` для ключа и для значения. Также приведен пример с использованием объектов кастомного класса в качестве ключей (при этом объекты также могут быть использованы и для значений, но этот кейс здесь не рассматривается).

Для вывода сообщений используется кастомный класс `MyLogger` на основе библиотеки `logging` (`chicago_spark/ChiSpark/mylog.py`)
...
Вывод: сравнивать можно (а иногда и нужно) по разным аттрибутам объектов.
...
Когда `dictdiffer` сравнивает значения по ключам в словарях, он вызывает метод `__eq__` для сравнения значений. Если метод `__eq__` не определен для конкретных объектов или возвращает `False`, `dictdiffer` считает эти значения различными.
